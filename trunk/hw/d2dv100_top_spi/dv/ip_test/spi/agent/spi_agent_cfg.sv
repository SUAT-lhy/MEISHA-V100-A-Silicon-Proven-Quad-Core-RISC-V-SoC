class spi_agent_cfg extends dv_base_agent_cfg;
  ...
  bit sck_polarity = 1'b0;       // aka CPOL
  bit sck_phase = 1'b0;          // aka CPHA
  bit host_bit_dir = 1'b0;       // 1 - lsb -> msb, 0 - msb -> lsb
  ...
  int num_bytes_per_trans_in_mon = 1;
  ...
  virtual task wait_sck_edge(sck_edge_type_e sck_edge_type);
    ...
    if (wait_posedge) @(posedge spi_if.sclk);
    else              @(negedge spi_if.sclk);
  endtask
