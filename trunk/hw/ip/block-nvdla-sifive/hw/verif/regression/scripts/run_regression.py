...
local_lib_path = os.path.join(os.path.dirname(__file__), "../../tools/local_python_libs") # 获取当前脚本所在目录的路径，并拼接上本地库目录名
if os.path.exists(local_lib_path): # 检查路径是否存在
    sys.path.insert(0, local_lib_path) # 将本地库目录添加到搜索路径的开头，优先级最高
else:
    print ("Fatal: path %s is cannot find" % local_lib_path)
    sys.exit(0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--timeout', '-timeout', dest='timeout', type=int, default=720000, required=False,
