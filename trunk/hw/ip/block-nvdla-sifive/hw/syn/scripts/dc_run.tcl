...
if {![file exists $SCRIPTS_DIR]} {
   puts "Error: Unable to proceed, please specify SCRITPS_DIR. "
   exit 1
}
...
if { $analyzeStatus != 1 } {
    puts "Error: Analyze failed!  Aborting..."
    exit 1
}
...
if ![ link ] {
    puts "Error: Failed to link. Aborting..."
    exit 1
}
...
# 使用通配符禁用特定库中的驱动能力小于2的INV和BUF 
#set_dont_use [get_lib_cells */*INV*X0]
set_dont_use [get_lib_cells */*INV*X1]
#set_dont_use [get_lib_cells */*BUF*X0]
set_dont_use [get_lib_cells */*BUF*X1]
set_dont_use [get_lib_cells */*INV*X0P*]
set_dont_use [get_lib_cells */*BUF*X0P*]

# Set dont use on user cells
if {[info exists DONT_USE_LIST] && (${DONT_USE_LIST} != "")} {
    set list_du [list]
	foreach tempCelltLib ${DONT_USE_LIST} {
        puts "${synMsgInfo} Putting dont_use attributes on cell ${tempCelltLib}]"
...
define_name_rules RegNameOpt -map {{"__","_"_"}}
change_names -rules RegNameOpt -hierarchy
