`define tc_name master_bus_soft_base_test
`define tc_name_cfg master_bus_soft_base_test_cfg
...
class `tc_name_cfg extends soft_base_test_cfg;
  ...
  virtual function void initialize(bit [TL_AW-1:0] csr_base_addr = '1);
	...
	foreach(this.axi_agent_sw[i]) begin
        ...
        if(uvm_is_match("*U_D2D_MASTER_BUS*s0_axi_slv*", i)) begin
        ...
        else if(uvm_is_match("*vc707mig1gb*s_axi_slv*", i)) begin
        ...
        `uvm_info(get_type_name(), $sformatf("set_vip_agent_work_mode for inst_path %s, work_mode %s", i, this.axi_agent_work_mode[i].name()), UVM_NONE);
...
function void `tc_name::build_phase(uvm_phase phase);
    ...
    env.cfg = cfg;
    this.DDR_MEM_REGEX_NAME = "topDesign*U_D2D_MASTER_BUS-amba_axi_m3s2*_m_bind_if_inst-s0_axi_slv";
