`define tc_name nvdla_soft_base_test
`define tc_name_cfg nvdla_soft_base_test_cfg
...
// max 100000
`define DDR_MEM_BYTE_DEPTH 32'h2_0000
//`define DDR_MEM_BASE_ADDR 32'h8000_0000  // mig axi slv port addr is begin from 0
// only use 29bit match backdoor load
// 32'h9000_0000
`define NVDLA_DT_MEM_OFFSET_ADDR 32'h1000_0000 
`define NVDLA_WT_MEM_OFFSET_ADDR 32'h1100_0000 
...
class `tc_name_cfg extends soft_base_test_cfg;
  ...
  virtual function void initialize(bit [TL_AW-1:0] csr_base_addr = '1);
	...
	foreach(this.axi_agent_sw[i]) begin 
        ...
        if(uvm_is_match("*mig*slave_0_slv*", i)) begin
            ...
            // donot force avoid backdoor load 0x9000_0000
...
class `tc_name extends soft_base_test;
    ...
    function new(string name, uvm_component parent);
        ...
        //this.DDR_MEM_REGEX_NAME = "topDesign*CHIPLINK_PREFIX_ChipLinkMaster*bind_if_inst-slave_0_slv";
        this.DDR_MEM_REGEX_NAME = "*fpga_ddr_chiplink*bind_if_inst-master_mem_0_mst";
        this.DDR_MEM_REGEX_NAME = "*mig*slave_0_slv*";
    ...
    extern task load_data_to_ddr(input bit [31:0] start_addr, input string load_file_path);
...
task `tc_name::load_data_to_ddr(input bit [31:0] start_addr, input string load_file_path);
    bit [`DDR_MEM_WIDTH - 1:0] mem_content[`DDR_MEM_BYTE_DEPTH];
    int mem_line_count;
    int fd;
    // 定义用于存储每行数据的字符串变量 
    string line;
	$readmemh(load_file_path, mem_content);
    // 打开文件 
    fd = $fopen(load_file_path, "r"); 
    if (fd) begin 
        // 逐行读取文件并计数行数 
        while (!$feof(fd)) begin 
            void'($fgets(line, fd)); 
        ...
        $fclose(fd);
        `uvm_info(get_type_name(), $sformatf("load_data_to_ddr: 文件路径: %0s, 文件行数: %d, 起始地址: %0h", load_file_path, mem_line_count, start_addr), UVM_NONE); 
    end 
    else begin 
        `uvm_error(get_type_name(), $sformatf("无法打开文件: %s", load_file_path));
    end

    if(`DDR_MEM_WIDTH == 64) begin
		for(int ram_depth_idx = 0; ram_depth_idx < mem_line_count; ram_depth_idx++) begin
			this.mem["ddr"].write_dword(start_addr + ram_depth_idx * 8, mem_content[ram_depth_idx]);
		...
		for(int ram_depth_idx = 0; ram_depth_idx < mem_line_count; ram_depth_idx++) begin
			this.mem["ddr"].write_word(start_addr + ram_depth_idx * 4, mem_content[ram_depth_idx]);
			`uvm_info(get_type_name(), $sformatf("load mem_content[%x]=%x", ram_depth_idx, mem_content[ram_depth_idx]), UVM_MEDIUM)
		end
	end
	else begin
...
task  `tc_name::backdoor_load_data(bit [31:0] ddr_mem_base_addr);
    ...
    begin
        ...
        this.load_data_to_ddr(ddr_mem_base_addr, OUT_SRAM_HEX_FILE);
    end

    begin
        string load_nvdla;
        // load nvdla dt
        if ($value$plusargs("load_nvdla_dt=%s", load_nvdla)) begin
            `uvm_info(get_type_name(), $sformatf("load_nvdla_dt=%s", load_nvdla), UVM_MEDIUM)
            // Check wheather the hex  exist
            if(!$fopen(load_nvdla, "r"))
                $fatal();
        end
        else begin 
            `uvm_error(get_type_name(), $sformatf("无法打开文件: %s", load_nvdla));
        end
        // for nvdla dt need use in 9000_0000, not overlap other addr
        this.load_data_to_ddr(ddr_mem_base_addr + `NVDLA_DT_MEM_OFFSET_ADDR, load_nvdla);
    end

    begin
        string load_nvdla;
        // load nvdla wt
        if ($value$plusargs("load_nvdla_wt=%s", load_nvdla)) begin
            `uvm_info(get_type_name(), $sformatf("load_nvdla_wt=%s", load_nvdla), UVM_MEDIUM)
            // Check wheather the hex  exist
            if(!$fopen(load_nvdla, "r"))
                $fatal();
        end
        ...
        // for nvdla wt need use in 9100_0000, not overlap other addr
        this.load_data_to_ddr(ddr_mem_base_addr + `NVDLA_WT_MEM_OFFSET_ADDR, load_nvdla);
    end
