`define tc_name spi_slave_base_test
`define tc_name_cfg spi_slave_base_test_cfg
  ...
  virtual function void initialize(bit [TL_AW-1:0] csr_base_addr = '1);
    ...
    foreach(this.axi_agent_sw[i]) begin
        if(uvm_is_match("*u_fpga_ddr_chiplink*master_mem_0_mst*", i)) begin
            ...
            this.axi_agent_sw[i] = dv_utils_pkg::OFF;
        end
...
class `tc_name extends tlul_base_test;
    ...
    logic [31:0] data = 0;
    logic [31:0] s_data;
    logic [63:0] stimuli [9999:0];
    logic [31:0] addr;
    logic [7:0]  actual_reg_val;
    logic [7:0]  reg_val;
    logic is_qspi = $urandom(); // 1'b1 for qspi, 1'b0 for standard spi
...
virtual task spi_load(input use_qspi);
    ...
    if ($value$plusargs("spi_stim_hex_file=%s", spi_stim_hex_file)) begin
        
    end
    else begin
        spi_stim_hex_file = "/home/autumn/test_new_meisha/spi_slave/MEISHAV100/trunk/hw/d2dv100_top/dv/ip_test/spi_slave/spi_stim.txt";
    end
    $display("spi_stim_hex_file = %s\n", spi_stim_hex_file);
    $readmemh(spi_stim_hex_file, stimuli);  //  read in the stimuli vectors  == address_value
    ...
    begin
        ...
        if (num_stim > 1 | stimuli[num_stim] === 64'bx)
            more_stim = 0;
    end
    #100 cfg.spi_slave_if.spi_csn = 1'b1;
    #`DELAY_BETWEEN_SPI;
...
virtual task spi_check(input use_qspi);
    ...
    begin
        ...
        `uvm_info(`gfn, $sformatf("SPI read data is %h, and the actual data is %h", spi_data_recv, spi_data), UVM_NONE)
...
virtual task spi_read_reg(input use_qspi, input [7:0] command, output [7:0] reg_val);
    begin
        cfg.spi_slave_if.spi_csn = 1'b0;
        #100;
        // send command
        ...
        begin
            for (int i = 2; i > 0; i--)
            begin   
                reg_val[4*i-1] = cfg.spi_slave_if.spi_sdi[3];
                reg_val[4*i-2] = cfg.spi_slave_if.spi_sdi[2];
                reg_val[4*i-3] = cfg.spi_slave_if.spi_sdi[1];
                reg_val[4*i-4] = cfg.spi_slave_if.spi_sdi[0];
                #`SPI_SEMIPERIOD cfg.spi_slave_if.spi_clk = 1'b1;
                #`SPI_SEMIPERIOD cfg.spi_slave_if.spi_clk = 1'b0;
            end
        end
            ...
            for (int i = 7; i >= 0; i--)
            begin
                reg_val[i] = cfg.spi_slave_if.spi_sdi[0];
                #`SPI_SEMIPERIOD cfg.spi_slave_if.spi_clk = 1'b1;
                #`SPI_SEMIPERIOD cfg.spi_slave_if.spi_clk = 1'b0;
            end
        end
...
virtual task spi_write_word(input use_qspi, input [31:0] addr, input [31:0] data);
    ...
    `uvm_info(`gfn, $sformatf("begin spi_write_word addr: %h, data: %h", addr, data), UVM_NONE)
    cfg.spi_slave_if.spi_csn = 1'b0;
    #`DELAY_BETWEEN_SPI;
    ...
    `uvm_info(`gfn, $sformatf("end spi_write_word addr: %h, data: %h", addr, data), UVM_NONE)
endtask

virtual task spi_read_nword(input use_qspi, input [31:0] addr, input int n, inout [31:0] data[]);
    logic [7:0] command;
    
    `uvm_info(`gfn, $sformatf("begin spi_read_nword addr: %h, n: %h", addr, n), UVM_NONE)
    command = 8'hB;
    ...
    `uvm_info(`gfn, $sformatf("end spi_read_nword addr: %h, n: %h", addr, n), UVM_NONE)
endtask
...
task `tc_name::main_phase(uvm_phase phase);
    ...
    begin
        ...
        // // test write/read reg
        // spi_read_reg(is_qspi, 8'h5, reg_val);
        // `uvm_info("SPI read reg0", $sformatf("reg0 value is %h", reg_val), UVM_NONE)

        // if (reg_val != 8'h1)
        //     `uvm_error("SPI read reg0", $sformatf("reg0 value is %h", reg_val))
        
        // for (int i = 0; i < 10; ++i)
        // begin
        //     actual_reg_val = $random;
        //     spi_write_reg(is_qspi, 8'h11, actual_reg_val);

        //     spi_read_reg(is_qspi, 8'h7, reg_val);
        //     `uvm_info("SPI read reg1", $sformatf("reg1 value is %h, the actual value is %h", reg_val, actual_reg_val), UVM_NONE)

        //     spi_write_reg(is_qspi, 8'h20, actual_reg_val);

        //     spi_read_reg(is_qspi, 8'h21, reg_val);
        //     `uvm_info("SPI read reg2", $sformatf("reg2 value is %h, the actual value is %h", reg_val, actual_reg_val), UVM_NONE)

        //     spi_write_reg(is_qspi, 8'h30, actual_reg_val);

        //     spi_read_reg(is_qspi, 8'h31, reg_val);
        //     `uvm_info("SPI read reg3", $sformatf("reg3 value is %h, the actual value is %h", reg_val, actual_reg_val), UVM_NONE)
        // end
        ...
        begin
            #100us;
            s_data = $random;
            spi_write_word(is_qspi, addr, s_data);
            #100us;
            spi_read_word(is_qspi, addr, data);
        ...
        // cfg.spi_slave_if.spi_csn = 1'b0;
        // #100;
        // spi_send_cmd_addr(is_qspi, 8'h2, 32'h8000_0000);
        // spi_send_data(is_qspi, 32'h8796_ac14);
        // #100 cfg.spi_slave_if.spi_csn = 1'b1;
        // #`DELAY_BETWEEN_SPI;
        // cfg.spi_slave_if.spi_csn = 1'b0;
        // #100;
        // spi_send_cmd_addr(is_qspi, 8'hB, 32'h8000_0000);
        // // dummy cycles
        // for (int i = 32; i >= 0; i--)
        // begin
        //     #`SPI_SEMIPERIOD cfg.spi_slave_if.spi_clk = 1;
        //     #`SPI_SEMIPERIOD cfg.spi_slave_if.spi_clk = 0;
        // end
        // spi_recv_data(is_qspi, data);
        // `uvm_info("SPI test", $sformatf("receive data is %h", data), UVM_NONE)
    end
    #1000ns;
