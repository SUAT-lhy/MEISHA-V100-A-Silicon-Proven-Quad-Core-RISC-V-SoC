`define tc_name sram_soft_base_test
`define tc_name_cfg sram_soft_base_test_cfg
...
//`define SRAM_MEM_HIR_str "u_MEISHAV100_TOP-topDesign-topMod-maskROM-TLMaskROM_bind_if_inst-tl_slv-sqr"
`define SRAM_MEM_HIR_str "tb.u_MEISHAV100_TOP.topDesign.topMod.d2d_sub.U_SOC_IRAM"
//ram_array_inst[0]/u_block_sp_sram/Memory_byte0[8191:0]
//`define SRAM_MEM_HIR device_seq[`BOOTROM_HIR_str].mem
`define SRAM_MEM_DEPTH 32'h4_0000
`define SRAM_MEM_WIDTH 64
`define SRAM_MEM_BYTE_DEPTH 32'h0_0000
`define SRAM_MEM_BASE_ADDR 32'h5000_0000



///tb/u_MEISHAV100_TOP/topDesign/topMod/pcie/axi_to_pcie_x1/blackbox/io_b2c_clk
//`define DDR_MEM_HIR env.m_axi_agent["u_MEISHAV100_TOP-topDesign-topMod-pcie-axi_to_pcie_x1-blackbox-chipLinkConverter-CHIPLINK_PREFIX_ChipLinkMaster_bind_if_inst-master_mem_0_mst"].s_drv.mem
//`define DDR_MEM_HIR env.m_axi_agent["u_fpga_ddr_chiplink-CHIPLINK_PREFIX_ChipLinkMaster_m_bind_if_inst-master_mem_0_mst"].s_drv.mem
...
class `tc_name_cfg extends soft_base_test_cfg;
  ...
  virtual function void initialize(bit [TL_AW-1:0] csr_base_addr = '1);
	...
	foreach(this.axi_agent_sw[i]) begin
        ...
        //this.m_axi_agent_c.fg[i].if_mode = dv_utils_pkg::Host;  // donnot need add in here
    end                      
...
class `tc_name extends soft_base_test;
    ...
    virtual task wait_clk_rst();
        fork
            begin // execute in pre_reset_phase
                #300ns; // wait dut_wrapper force complete， before allon reset_release, after top wrapper force 0 200ns
...
task `tc_name::main_phase(uvm_phase phase);
    ...
    fork
        begin
            // release core rst
            assert(uvm_hdl_force("tb.u_MEISHAV100_TOP.topDesign.topMod.d2d_sub.U_SOC_LSYS.all_tile_sft_rst", 1'b0)); 
            #10ms;  // wait core done
...
task `tc_name::backdoor_load_data(bit [31:0] ddr_mem_base_addr);
    ...
    // unuse bootrom
	if(device_seq.exists(`BOOTROM_HIR_str) & 0) begin
...
/////////////////////////////////////////////////////////////////////
	...
	if(`SRAM_MEM_WIDTH == 64) begin
        int ram_bank_idx = 0;
        int ram_depth_idx = 0;
        string split_str = ".";
		for(int bin_ram_depth_idx = 0; bin_ram_depth_idx < ddr_mem_line_count; bin_ram_depth_idx++) begin
            ram_bank_idx = bin_ram_depth_idx/65536;
            ram_depth_idx = bin_ram_depth_idx%65536;
            split_str = $sformatf("[%0d].", ram_bank_idx);
            `ifdef POSTSIM_EN
                split_str = $sformatf("_%0d_", ram_bank_idx);
            `endif
            // load data to sram vendor ram
            assert(uvm_hdl_deposit($sformatf("%0s.ram_array_inst%0su_block_sp_sram.Memory_byte0[%0d]", `SRAM_MEM_HIR_str, split_str, ram_depth_idx), mem_content[ram_depth_idx][7:0]));
            assert(uvm_hdl_deposit($sformatf("%0s.ram_array_inst%0su_block_sp_sram.Memory_byte1[%0d]", `SRAM_MEM_HIR_str, split_str, ram_depth_idx), mem_content[ram_depth_idx][15:8]));
            assert(uvm_hdl_deposit($sformatf("%0s.ram_array_inst%0su_block_sp_sram.Memory_byte2[%0d]", `SRAM_MEM_HIR_str, split_str, ram_depth_idx), mem_content[ram_depth_idx][23:16]));
            assert(uvm_hdl_deposit($sformatf("%0s.ram_array_inst%0su_block_sp_sram.Memory_byte3[%0d]", `SRAM_MEM_HIR_str, split_str, ram_depth_idx), mem_content[ram_depth_idx][31:24]));
            assert(uvm_hdl_deposit($sformatf("%0s.ram_array_inst%0su_block_sp_sram.Memory_byte4[%0d]", `SRAM_MEM_HIR_str, split_str, ram_depth_idx), mem_content[ram_depth_idx][39:32]));
            assert(uvm_hdl_deposit($sformatf("%0s.ram_array_inst%0su_block_sp_sram.Memory_byte5[%0d]", `SRAM_MEM_HIR_str, split_str, ram_depth_idx), mem_content[ram_depth_idx][47:40]));
            assert(uvm_hdl_deposit($sformatf("%0s.ram_array_inst%0su_block_sp_sram.Memory_byte6[%0d]", `SRAM_MEM_HIR_str, split_str, ram_depth_idx), mem_content[ram_depth_idx][55:48]));
            assert(uvm_hdl_deposit($sformatf("%0s.ram_array_inst%0su_block_sp_sram.Memory_byte7[%0d]", `SRAM_MEM_HIR_str, split_str, ram_depth_idx), mem_content[ram_depth_idx][63:56]));
			//`SRAM_MEM_HIR.write_dword(`SRAM_MEM_BASE_ADDR + ram_depth_idx * 8, mem_content[ram_depth_idx]);
			`uvm_info(get_type_name(), $sformatf("load mem_content_bank[%x][%x]=%x", ram_bank_idx, ram_depth_idx, mem_content[ram_depth_idx]), UVM_MEDIUM)
		end
	end
	//else if(`SRAM_MEM_WIDTH == 32) begin
		//for(int ram_depth_idx = 0; ram_depth_idx < ddr_mem_line_count; ram_depth_idx++) begin
			//`SRAM_MEM_HIR.write_word(`SRAM_MEM_BASE_ADDR + ram_depth_idx * 4, mem_content[ram_depth_idx]);
			//`uvm_info(get_type_name(), $sformatf("load mem_content[%x]=%x", ram_depth_idx, mem_content[ram_depth_idx]), UVM_MEDIUM)
		//end
	//end
	else begin
		`uvm_error(get_type_name(), $sformatf("load mem_content error, the mem_width %0d is undefined", `SRAM_MEM_WIDTH))
	end
...
`undef SRAM_MEM_HIR
`undef SRAM_MEM_DEPTH
`undef SRAM_MEM_WIDTH
`undef SRAM_MEM_BYTE_DEPTH
`undef SRAM_MEM_BASE_ADDR
