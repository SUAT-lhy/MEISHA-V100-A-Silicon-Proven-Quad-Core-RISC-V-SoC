...
`POLY_CLASS_UTILS_DEFINE_BEGIN(tl_if, tl_port_info, tl_intf_c)
    ...
    function new();
        ...
        foreach(port_index_name_map[i]) begin
            ...
            tl_if_port_en[i] = 0;  // force port_en 0, avoid init fail
        end
        ...
        //if(port_type == tl_intf_c::PORT_TYPE) begin
            `uvm_info("set_tl_vif_work_mode", $sformatf("switch %0d, instance %s set_vif_work_mode %0d, port %s", switch, inst_path, work_mode, port_name), UVM_DEBUG);
