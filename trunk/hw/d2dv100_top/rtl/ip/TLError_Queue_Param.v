...
`endif // RANDOMIZE_REG_INIT
  ...
  reg [SOURCE_WIDTH-1:0] ram_source;
  ...
  assign ram_opcode_io_deq_bits_MPORT_data = ram_opcode;
  assign ram_opcode_MPORT_data = io_enq_bits_opcode;
  assign ram_opcode_MPORT_addr = 1'h0;
  assign ram_opcode_MPORT_mask = 1'h1;
  assign ram_opcode_MPORT_en = io_enq_ready & io_enq_valid;
  assign ram_size_io_deq_bits_MPORT_addr = 1'h0;
  assign ram_size_io_deq_bits_MPORT_data = ram_size;
  assign ram_size_MPORT_data = io_enq_bits_size;
  assign ram_size_MPORT_addr = 1'h0;
  assign ram_size_MPORT_mask = 1'h1;
  assign ram_size_MPORT_en = io_enq_ready & io_enq_valid;
  assign ram_source_io_deq_bits_MPORT_addr = 1'h0;
  assign ram_source_io_deq_bits_MPORT_data = ram_source;
  ...
  always @(posedge clock) begin
    if(ram_opcode_MPORT_en & ram_opcode_MPORT_mask) begin
      ram_opcode <= ram_opcode_MPORT_data;
    end
    if(ram_size_MPORT_en & ram_size_MPORT_mask) begin
      ram_size <= ram_size_MPORT_data;
    end
    if(ram_source_MPORT_en & ram_source_MPORT_mask) begin
      ram_source <= ram_source_MPORT_data;
    end
    if (reset) begin
      maybe_full <= 1'h0;
...
`ifdef RANDOMIZE_MEM_INIT
  ...
  for (initvar = 0; initvar < 1; initvar = initvar+1)
    ram_opcode = _RAND_0[2:0];
  _RAND_1 = {1{`RANDOM}};
  for (initvar = 0; initvar < 1; initvar = initvar+1)
    ram_size = _RAND_1[2:0];
  _RAND_2 = {1{`RANDOM}};
  for (initvar = 0; initvar < 1; initvar = initvar+1)
    ram_source = _RAND_2[SOURCE_WIDTH-1:0];
`endif // RANDOMIZE_MEM_INIT
`ifdef RANDOMIZE_REG_INIT
  _RAND_3 = {1{`RANDOM}};
