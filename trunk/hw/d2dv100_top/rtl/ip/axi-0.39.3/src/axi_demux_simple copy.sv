...
module axi_demux_simple #(
  ...
  // parameter int unsigned SelectWidth    = `IDX_WIDTH_LOG2(NoMstPorts),
  parameter int unsigned SelectWidth    = 1, // for fix 2
  parameter type         select_t       = logic // for fix 2
) (
  input  logic                          clk_i,
  input  logic                          rst_ni,
  input  logic                          test_i,
  ...
  //localparam int unsigned IdCounterWidth = `IDX_WIDTH_LOG2(MaxTrans);
  localparam int unsigned IdCounterWidth = 4;
  typedef logic [IdCounterWidth-1:0] id_cnt_t;

  `ifndef SYNTHESIS
    initial begin
      #10ns;
      if (NoMstPorts != 2) begin 
        $error("%m: NoMstPorts %0d is not 2, because vcs 2016 cannot support", NoMstPorts);
      end
      if (MaxTrans != 16) begin 
        $error("%m: MaxTrans %0d is not 16, because vcs 2016 cannot support", MaxTrans);
      end
      if ((AxiLookBits != 4) && (AxiLookBits != 1)) begin 
        $error("%m: AxiLookBits %0d is not 1 or 4, because vcs 2016 cannot support", AxiLookBits);
      end
  ...
  if (NoMstPorts == 32'h1) begin : gen_no_demux
    ...
    if (UniqueIds) begin : gen_unique_ids_aw
      ...
      axi_demux_id_counters #(
        ...
        .clk_i                        ( clk_i                          ),
        .rst_ni                       ( rst_ni                         ),
        .lookup_axi_id_i              ( slv_req_i.aw.id[0+:AxiLookBits] ),  // vcs 2016 slv_req_i.aw.id[0+:AxiLookBits]
    ...
    //  B Channel
    //--------------------------------------
    //logic [`IDX_WIDTH_LOG2(NoMstPorts)-1:0] b_idx;
    logic b_idx;
    ...
    if (UniqueIds) begin : gen_unique_ids_ar
      ...
      axi_demux_id_counters #(
        //.mst_port_select_t ( select_t       ),
        .AxiIdBits         ( AxiLookBits    ),  // 
        .CounterWidth      ( 4    )     // IdCounterWidth
      ) i_ar_id_counter (
        .clk_i                        ( clk_i                                       ),
        .rst_ni                       ( rst_ni                                      ),
        .lookup_axi_id_i              ( slv_req_i.ar.id[0+:AxiLookBits]                       ),  // vcs 2016 
    ...
    //logic [`IDX_WIDTH_LOG2(NoMstPorts)-1:0] r_idx;
    logic r_idx;
...
module axi_demux_id_counters #(
  ...
  `ifndef SYNTHESIS
    initial begin
      if (CounterWidth != 4) begin 
        $error("%m: CounterWidth parameter is not 4, because vcs 2016 cannot support");
      end
