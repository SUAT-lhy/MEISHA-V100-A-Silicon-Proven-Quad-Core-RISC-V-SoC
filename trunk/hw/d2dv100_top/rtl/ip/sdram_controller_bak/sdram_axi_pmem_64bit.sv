...
(
    ...
    ,input  [ WIDTH_STRB-1:0]  axi_wstrb
    ,input           axi_wlast
    ,input           axi_bready
    ,input           axi_arvalid
...
begin
    mask = 0;
    asize_bytes = (axsize < $clog2(WIDTH_STRB)) ? (1 << axsize) : (1 << $clog2(WIDTH_STRB));
    case (axtype)
    ...
    begin
        ...
        calculate_addr_next = (addr & ~mask) | ((addr + asize_bytes) & mask);
    end
    default: // AXI4_BURST_INCR
        calculate_addr_next = addr + asize_bytes;
        //calculate_addr_next = addr + WIDTH_STRB;
    endcase
end
endfunction
...
always @ (posedge clk_i or posedge rst_i) begin
    if (rst_i) begin
        ...
        begin
            ...
            begin
                req_addr_q <= calculate_addr_next(req_addr_q, req_axburst_q, req_axlen_q, req_size_q);
        ...
        if (axi_awvalid && axi_awready) begin
            ...
            begin
                ...
                req_size_q    <= axi_awsize;
                req_id_q      <= axi_awid;
                req_axburst_q <= axi_awburst;
                req_axlen_q   <= axi_awlen;
                req_addr_q    <= calculate_addr_next(axi_awaddr, axi_awburst, axi_awlen, axi_awsize);
            end
            // Data not ready
            else
            begin
        ...
        begin
            ...
            req_size_q    <= axi_arsize;
            req_addr_q    <= calculate_addr_next(axi_araddr, axi_arburst, axi_arlen, axi_arsize);
            req_id_q      <= axi_arid;
...
assign ram_len_o        = axi_awvalid ? axi_awlen:
                          axi_arvalid ? axi_arlen : 8'b0;
...
assign axi_rlast   = resp_is_last_w;
