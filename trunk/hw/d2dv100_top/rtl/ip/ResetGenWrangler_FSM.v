module ResetGenWrangler_FSM  # (parameter RESET_CNT_WIDTH = 16) ( 
  input   debug_mode_sel_pad_glitch_free,
  input   auto_in_chiplink_clock,  // from d2d_sub d2d_clock clk_main_clk
  input   auto_in_ddr_clock,  // from d2d_sub d2d_clock clk_main_clk
...
`ifdef CRG_SPEED_UP
  ...
  localparam TILE_RLS_CNT     = 16'h0fe0;
  
`else

  localparam RESET_CNT   = 16'h500;

  localparam MAIN_RLS_CNT     = 16'h2500;
  localparam CHIPLINK_RLS_CNT = 16'h3700;
  localparam TILE_RLS_CNT     = 16'h9fe0;
...
`endif
  ...
  wire  auto_out_ddr_reset_sync_reset;
  ...
  ResetCatchAndSync_d3 ddr_reset_ResetCatchAndSync_d3 ( 
    .clock(auto_in_ddr_clock),
    .reset(auto_out_ddr_reset_sync_reset),
    .io_sync_reset(auto_out_ddr_reset)
  );
  ...
  assign auto_out_main_reset_sync_reset = ~((pwr_cur_st == PWR_RLS_STATE) &&  (pwr_rls_cnt >= MAIN_RLS_CNT));
  assign auto_out_chiplink_reset_sync_reset = ~((pwr_cur_st == PWR_RLS_STATE) &&  (pwr_rls_cnt >= CHIPLINK_RLS_CNT));
  assign auto_out_ddr_reset_sync_reset = ~((pwr_cur_st == PWR_RLS_STATE) &&  (pwr_rls_cnt >= CHIPLINK_RLS_CNT));
