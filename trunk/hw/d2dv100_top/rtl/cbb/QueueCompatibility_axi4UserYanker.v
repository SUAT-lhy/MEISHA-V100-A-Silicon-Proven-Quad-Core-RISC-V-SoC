module QueueCompatibility_axi4UserYanker #(
  ...
  output       io_enq_ready,
  input        io_enq_valid,
  input  [AXI4_ID_WIDTH-2:0] io_enq_bits_extra_id,
  input        io_enq_bits_real_last,
  input        io_deq_ready,
  output       io_deq_valid,
  output [AXI4_ID_WIDTH-2:0] io_deq_bits_extra_id,
...
`endif // RANDOMIZE_REG_INIT
  reg [AXI4_ID_WIDTH-2:0] ram_extra_id [0:3]; // @[Decoupled.scala 259:95]
  ...
  wire [AXI4_ID_WIDTH-2:0] ram_extra_id_io_deq_bits_MPORT_data; // @[Decoupled.scala 259:95]
  wire [AXI4_ID_WIDTH-2:0] ram_extra_id_MPORT_data; // @[Decoupled.scala 259:95]
...
`ifdef RANDOMIZE_MEM_INIT
  ...
  for (initvar = 0; initvar < 4; initvar = initvar+1)
    ram_extra_id[initvar] = _RAND_0[AXI4_ID_WIDTH-2:0];
  _RAND_1 = {1{`RANDOM}};
  for (initvar = 0; initvar < 4; initvar = initvar+1)
