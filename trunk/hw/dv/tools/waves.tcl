...
if {[info exists ::env(WAVE_FILE_NAME)]} {
  set wavedump_db "$::env(WAVE_FILE_NAME).$waves"
} else {
  puts "WARNING: wavedump_db environment variable not set - using waves.fsdb as dumpfsdb"
}
...
proc wavedumpScope {scope fid {depth 0} {fsdb_flags "+all"} {probe_flags  "-all"}
  ...
  switch $waves {
    ...
    "fsdb" {
      ...
      if {$simulator eq "xcelium"} {
        call fsdbDumpvars $depth $scope $fsdb_flags "+mda"
        call fsdbDumpSVA $depth $scope
      } else {
        fsdbDumpvars $depth $scope $fsdb_flags "+mda"
