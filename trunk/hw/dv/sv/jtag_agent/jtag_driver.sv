...
class jtag_driver extends dv_base_driver #(jtag_item, jtag_agent_cfg);
  ...
  virtual task reset_signals();
    if (cfg.if_mode == Host) begin
      ...
      `HOST_CB.tms <= 1'b0;
      `HOST_CB.tdi <= 1'b0;
      cfg.vif.wait_unreset_state();  
      cfg.vif.ports_reset();
      cfg.vif.goto_reset_state();
      cfg.vif.goto_run_test_idle();
    end
    else begin
      `DEVICE_CB.tdo <= 1'b0;
    end
  ...
  virtual task get_and_drive_host_mode();
    forever begin
      ...
      cfg.set_tck_en(1'b0);
      if (!req.select_ir) begin
        `uvm_info(`gfn, $sformatf("item sent:\n%0s", rsp.sprint()), UVM_LOW)
      end
  ...
  task drive_jtag_ir(int len, bit [JTAG_DRW-1:0] ir);
    ...
    `HOST_CB.tdi <= 1'b0;
    @(`HOST_CB);
    cfg.vif.goto_run_test_idle(cfg.idle_delay_cycle);
  endtask
