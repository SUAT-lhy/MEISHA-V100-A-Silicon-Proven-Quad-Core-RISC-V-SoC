...
static void reg_write(unsigned int addr, unsigned int val){
  _REG32(NVDLA_BASE, addr) = val;
}

static unsigned int  reg_read(unsigned int addr){
  return _REG32(NVDLA_BASE, addr);
}

static void cfg_pll_para(int cfg) {

	kprintf("Info: test pll state index: 0x%x", _REG32(_AC(0x51000000, UL), 0x20));

    _REG32(_AC(0x51000000, UL), 0x18) = (0x1 << 15) + (0x1 << 10) + (0x1 << 5) + (0x1);  // SOC_LSYS cfg pll cfg, use pll output default clk
}

static void cfg_nvdla_rst(int cfg) {
...
int main(void)
{
    ...
    _REG32(_AC(0x51000000, UL), 0x18) = (0x1 << 15) + (0x1 << 10) + (0x1 << 5) + (0x1);  // SOC_LSYS cfg
    //cfg_nvdla_rst(0);
    ...
    kprintf("cycle1: %x, cycle2: %x, diff: %x\n", cycle1, cycle2, cycle2 - cycle1 );
